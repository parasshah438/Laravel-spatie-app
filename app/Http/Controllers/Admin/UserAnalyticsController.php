...
class UserAnalyticsController extends Controller
{
    ...
    public function export(Request $request)
    {
        $startDate = $request->get('start_date', now()->subDays(30)->format('Y-m-d'));
        $endDate = $request->get('end_date', now()->format('Y-m-d'));
        $format = $request->get('format', 'excel'); // excel, csv, json
        ...
        $filename = "user_analytics_{$startDate}_to_{$endDate}";
        
        switch ($format) {
            case 'csv':
                return $this->exportCsv($analytics, $startDate, $endDate, $filename);
            case 'json':
                return $this->exportJson($analytics, $startDate, $endDate, $filename);
            case 'excel':
            default:
                return $this->exportExcel($analytics, $startDate, $endDate, $filename);
        }
    }
     ...
     */
    private function exportExcel($analytics, $startDate, $endDate, $filename)
    {
        // For now, export as CSV since we don't have Excel package
        return $this->exportCsv($analytics, $startDate, $endDate, $filename);
    }
     ...
     * Export as CSV file
     */
    private function exportCsv($analytics, $startDate, $endDate, $filename)
    {
        $csvData = [];
        
        // Header
        $csvData[] = ['User Registration Analytics Report'];
        $csvData[] = ['Period', "{$startDate} to {$endDate}"];
        $csvData[] = ['Generated on', now()->format('Y-m-d H:i:s')];
        $csvData[] = ['Generated by', auth('admin')->user()->name];
        $csvData[] = []; // Empty row
        
        // Summary data
        $csvData[] = ['Metric', 'Value'];
        $csvData[] = ['Total Users', $analytics['total_users']];
        $csvData[] = ['Users in Period', $analytics['users_in_period']];
        $csvData[] = ['Users Today', $analytics['users_today']];
        $csvData[] = ['Users This Month', $analytics['users_this_month']];
        $csvData[] = ['Total Customers', $analytics['total_customers']];
        $csvData[] = ['Customers in Period', $analytics['customers_in_period']];
        $csvData[] = ['Customers Today', $analytics['customers_today']];
        $csvData[] = ['Customers This Month', $analytics['customers_this_month']];
        $csvData[] = ['User Growth Rate (%)', $analytics['user_growth_rate']];
        $csvData[] = ['Customer Growth Rate (%)', $analytics['customer_growth_rate']];
        $csvData[] = ['Total Activities in Period', $analytics['total_activities_in_period']];
        
        if ($analytics['most_active_day']) {
            $csvData[] = ['Most Active Day', $analytics['most_active_day']['date']];
            $csvData[] = ['Most Active Day Count', $analytics['most_active_day']['count']];
            $csvData[] = ['Most Active Day Type', $analytics['most_active_day']['type']];
        }
     ...
     * Export as JSON file
     */
    private function exportJson($analytics, $startDate, $endDate, $filename)
    {
        $jsonData = [
            'report_title' => 'User Registration Analytics Report',
            'period' => [
                'start_date' => $startDate,
                'end_date' => $endDate
            ],
